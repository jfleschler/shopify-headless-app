...
export const testCartCreation = async () => {
  ...
  try {
    ...
    const result = await window.ShopifyHeadlessApp.Checkout.createCart(testCartItems);
    ...
    if (error.message.includes('Unauthorized') || error.message.includes('401')) {
      console.log('💡 This might be a credential issue. Make sure your STOREFRONT_TOKEN is valid.');
    }
    
    // Check if it's a variant ID issue
    if (error.message.includes('variant') || error.message.includes('merchandise')) {
      console.log('💡 This might be an invalid variant ID. Make sure you\'re using real product variant IDs.');
    }
...
export const testLocalCartCheckout = async () => {
  ...
  try {
    ...
    window.ShopifyHeadlessApp.Cart.addToCart('gid://shopify/ProductVariant/12345', 1);
    window.ShopifyHeadlessApp.Cart.addToCart('gid://shopify/ProductVariant/67890', 2);
    
    console.log('📦 Local cart items:', window.ShopifyHeadlessApp.Cart.getCart());
  ...
  cacheKeys.forEach(key => {
    ...
    if (data) {
      try {
        ...
        const itemCount = key === 'shopify-cache-collectionsList' 
          ? (parsed ? 1 : 0)
...
export const testCacheFunctionality = async () => {
  ...
  try {
    ...
    const collections = await window.ShopifyHeadlessApp.Collections.fetchCollections();
    console.log('✅ Collections fetched and cached:', collections.length, 'items');
    ...
    const searchResults = await window.ShopifyHeadlessApp.Search.searchProducts('test');
...
if (typeof window !== 'undefined') {
  ...
  console.log('  - CartTests.testCacheFunctionality() - Test cache persistence');
}
